<template>
    <div>
        <div v-for="(activity, index) in activities">
            <comment :comment="activity.subject.subject" :endpoint="'/comment/' + activity.subject.subject.id" login-endpoint="this.login" :comment-creator-endpoint="'/p/' + activity.subject.subject.creator.username" @deleted="remove(index)" :profileView="true"></comment>
        </div>
